...
func can_go(x, y, dir, n_iter):
	var w = dimension_.x
	var h = dimension_.y
	match dir:
		Dir.LEFT:
			return x > 0 && !is_left(x,y) && marks_array_[translate2index(x-1,y)] == n_iter
		Dir.RIGHT:
			return x < w-1 && !is_left(x+1,y) && marks_array_[translate2index(x+1,y)] == n_iter
		Dir.UP:
			return y > 0 && !is_top(x,y) && marks_array_[translate2index(x,y-1)] == n_iter
		Dir.DOWN:
			return y < h-1 && !is_top(x,y+1) && marks_array_[translate2index(x,y+1)] == n_iter

func mark_neighbour(x, y, dir, n_iter):
	match dir:
		Dir.LEFT:  rep_[translate2index(x-1,y)] = n_iter
		Dir.RIGHT: rep_[translate2index(x+1,y)] = n_iter
		Dir.UP:    rep_[translate2index(x,y-1)] = n_iter
		Dir.DOWN:  rep_[translate2index(x,y+1)] = n_iter
